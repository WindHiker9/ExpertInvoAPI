...
namespace ExpertInvoAPI.Controllers
{
    ...
    public class PlcController: ControllerBase
    {
        ...
        [HttpGet]
        [Route("api/plcController/all")]
        public IEnumerable<PlcModel> GetEntries()
        {
            return _Context.Plcdatatest;
        }
        ...
        public async Task<IActionResult> GetEntries([FromRoute] int id)
        {
            ...
            var entry = await _Context.Plcdatatest.SingleOrDefaultAsync(m => m.Id == id);
            if (entry == null)
            {
                return NotFound();
            }
        ...
        {
            return _Context.Plcdatatest.Any(e => e.Id == id);
        }
        ...
        public async Task<IActionResult> DeleteEntry([FromRoute] int id)
        {
            ...
            var data = await _Context.Plcdatatest.SingleOrDefaultAsync(m => m.Id == id);
            if(Entry == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> PostEntry([FromBody] PlcModel entry)
        {
            ...
            _Context.Plcdatatest.Add(entry);
            var result = _Context.Add(entry);
            await _Context.SaveChangesAsync(); //Saves entries
